...
##  3）离散图，运行不出来，待优化
    ...
    result = pd.DataFrame()
    test = pd.read_csv(r"C:\Users\a\Desktop\wk\0 个人数据挖掘项目计划\2 信用卡违约预测\0 数据集\test.csv")
    result['ID'] = test['ID']
    result['Target'] = ret1['Target']

    result.to_csv(r"C:\Users\a\Desktop\wk\0 个人数据挖掘项目计划\2 信用卡违约预测\0 数据集\result.csv", index=False)
