...
if __name__ == '__main__':
    ...
    train_X_base = train_Base.drop(columns=['policy_id', 'fraud'])
    train_Feature_base = train_Base['fraud']
    print(train_Feature_base)
    ...
    ## 3. 模型训练
    ## 3.1 建立模型
    gbm = LGBMClassifier(n_estimators=600, learning_rate=0.03, boosting_type='gbdt',
    ...
    for index_train, index_yanzheng in kf.split(train_X_base):
        #     print(index_train)
        #     print('------------------')
        #     print(index_yanzheng)
        X_train = train_X_base.iloc[index_train]
        y_train = train_Feature_base.iloc[index_train]
        #     print(train_X)
        X_yanzheng = train_X_base.iloc[index_yanzheng]
        y_yanzheng = train_Feature_base.iloc[index_yanzheng]
        ## 3.2 模型训练
        gbm.fit(X_train, y_train)
        ## 3.3 模型预测
        X_yanzheng_pred = gbm.predict_proba(X_yanzheng)[:, 1]
        print('验证集AUC:{}'.format(roc_auc_score(y_yanzheng, X_yanzheng_pred)))
    ...
    ## submission_Base.to_csv(r"data/submission.csv", index=False)
